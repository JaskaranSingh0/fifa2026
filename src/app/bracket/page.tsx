  ...
  useEffect(() => {
    ...
    async function loadMatches() {
      try {
        ...
        if (alive) setLiveMatches(data.matches || []);
      } catch (err) {
        console.error(err);
      } finally {
    ...
    loadMatches();
    // Poll so background-synced knockout results appear without a manual refresh.
    const id = setInterval(loadMatches, 20000);
      ...
      >
        ...
        {!loading && (
          <div className="mt-10 md:mt-14">
            <div className="relative flex flex-col items-center text-center" style={{ padding: "clamp(2rem,5vw,3.5rem) 0" }}>
              ...
              {champion ? (
                <div className="relative flex flex-col items-center">
                  <span className="uppercase" style={{ fontSize: "0.65rem", letterSpacing: "0.4em", color: heroAccent, fontWeight: 600 }}>
                    World Champions
                  </span>
            ...
            {/* Round-progress rail */}
            <div className="flex items-center gap-3" style={{ maxWidth: "640px", margin: "0 auto" }}>
              {STAGES.map((s, i) => (
